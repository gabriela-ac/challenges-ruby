...
describe Sql do
  ...
  it 'Realiza a query em Livros e retorna apenas os títulos dos livros' do
    ...
    expect(resultado).to eq [["Orgulho e preconceito"], ["Sejamos todos feministas"], ["O Diário de Anne Frank"], ["A Vida Mentirosa dos Adultos"], ["Quarto de Despejo: Diário de uma Favelada"]]
  end

  it 'Realiza a query em Livros e retorna apenas cada editora uma vez' do
    resultado = db.execute(Sql.new.query_editora)
    expect(resultado).to eq [["Cia. Letras"], ["Martina"], ["Fantástica"]]
  end
